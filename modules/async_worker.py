...
def worker():
    ...
    from modules.meta_parser import MetadataScheme, get_metadata_parser
    ...
    def handler(async_task):
        ...
        save_metadata_to_images = (
            args.pop() if not args_manager.args.disable_metadata else False
        )
        metadata_scheme = (
        ...
        if input_image_checkbox:
            if (
                (
                    current_tab == "uov"
                    or (current_tab == "ip" and mixing_image_prompt_and_vary_upscale)
                )
                and uov_method != flags.disabled
                and uov_input_image is not None
            ):
            ...
            if (
                current_tab == "inpaint"
                or (current_tab == "ip" and mixing_image_prompt_and_inpaint)
            ) and isinstance(inpaint_input_image, dict):
                inpaint_image = inpaint_input_image["image"]

                if isinstance(inpaint_mask_image_upload, np.ndarray):
                    if inpaint_mask_image_upload.ndim == 2:
                        H, W, C = inpaint_image.shape
                        inpaint_mask_image_upload = resample_image(
                            inpaint_mask_image_upload, width=W, height=H
                        )
                        inpaint_mask_image_upload = (
                            inpaint_mask_image_upload > 127
                        ).astype(np.uint8) * 255
                        inpaint_mask = inpaint_mask_image_upload

                if int(inpaint_erode_or_dilate) != 0:
                    inpaint_mask = erode_or_dilate(
                        inpaint_mask, inpaint_erode_or_dilate
                    )
            ...
            if (
                current_tab == "ip"
                or mixing_image_prompt_and_vary_upscale
                or mixing_image_prompt_and_inpaint
            ):
                goals.append("cn")
                progressbar(async_task, 1, "Downloading control models ...")
                if len(cn_tasks[flags.cn_canny]) > 0:
                    controlnet_canny_path = (
                        modules.config.downloading_controlnet_canny()
                    )
        ...
        if not skip_prompt_processing:
            ...
            lora_filenames = modules.util.remove_performance_lora(
                modules.config.lora_filenames, performance_selection
            )
            ...
            for i in range(image_number):
                ...
                task_negative_prompt = apply_wildcards(
                    negative_prompt, task_rng, i, read_wildcards_in_order
                )
                task_extra_positive_prompts = [
                    apply_wildcards(pmt, task_rng, i, read_wildcards_in_order)
        ...
        if "upscale" in goals:
            ...
            if direct_return:
                ...
                uov_input_image_path = log(
                    uov_input_image, d, output_format=output_format
                )
                yield_result(
                    async_task,
            ...
            if len(outpaint_selections) > 0:
                ...
                if "top" in outpaint_selections:
                    ...
                    inpaint_image = np.pad(
                        inpaint_image, [[distance_top, 0], [0, 0], [0, 0]], mode="edge"
                    )
                    inpaint_mask = np.pad(
                        inpaint_mask,
                        [[distance_top, 0], [0, 0]],
                ...
                if "bottom" in outpaint_selections:
                    ...
                    inpaint_image = np.pad(
                        inpaint_image,
                        [[0, distance_bottom], [0, 0], [0, 0]],
                        mode="edge",
                    )
                    inpaint_mask = np.pad(
                        inpaint_mask,
                        [[0, distance_bottom], [0, 0]],
                        mode="constant",
                        constant_values=255,
                    )
                ...
                if "left" in outpaint_selections:
                    ...
                    inpaint_image = np.pad(
                        inpaint_image, [[0, 0], [distance_left, 0], [0, 0]], mode="edge"
                    )
                    inpaint_mask = np.pad(
                        inpaint_mask,
                        [[0, 0], [distance_left, 0]],
                ...
                if "right" in outpaint_selections:
                    ...
                    inpaint_image = np.pad(
                        inpaint_image,
                        [[0, 0], [0, distance_right], [0, 0]],
                        mode="edge",
                    )
                    inpaint_mask = np.pad(
                        inpaint_mask,
                        [[0, 0], [0, distance_right]],
                        mode="constant",
                        constant_values=255,
                    )
            ...
            for task in cn_tasks[flags.cn_canny]:
                ...
                if not skipping_cn_preprocessor:
                    cn_img = preprocessors.canny_pyramid(
                        cn_img, canny_low_threshold, canny_high_threshold
                    )
        ...
        if freeu_enabled:
            ...
            pipeline.final_unet = core.apply_freeu(
                pipeline.final_unet, freeu_b1, freeu_b2, freeu_s1, freeu_s2
            )
        ...
        if scheduler_name in ["lcm", "tcd"]:
            ...
            def patch_discrete(unet):
                return core.opModelSamplingDiscrete.patch(
                    pipeline.final_unet, sampling=scheduler_name, zsnr=False
                )[0]
            ...
            def patch_edm(unet):
                return core.opModelSamplingContinuousEDM.patch(
                    unet, sampling=scheduler_name, sigma_max=120.0, sigma_min=0.002
                )[0]
        ...
        async_task.yields.append(
            ["preview", (flags.preparation_step_count, "Moving model to GPU ...", None)]
        )
        ...
        def callback(step, x0, x, total_steps, y):
            ...
            async_task.yields.append(
                [
                    ...
                    (
                        int(
                            flags.preparation_step_count
                            + (100 - flags.preparation_step_count)
                            * float(done_steps)
                            / float(all_steps)
                        ),
                        f"Sampling step {step + 1}/{total_steps}, image {current_task_id + 1}/{image_number} ...",
                        y,
                    ),
                ]
            )
        ...
        for current_task_id, task in enumerate(tasks):
            current_progress = int(
                flags.preparation_step_count
                + (100 - flags.preparation_step_count)
                * float(current_task_id * steps)
                / float(all_steps)
            )
            progressbar(
                async_task,
                current_progress,
                f"Preparing task {current_task_id + 1}/{image_number} ...",
            )
            execution_start_time = time.perf_counter()
            ...
            try:
                ...
                progressbar(
                    async_task,
                    current_progress,
                    f"Saving image {current_task_id + 1}/{image_number} to system ...",
                )
                for x in imgs:
                    d = [
                    ...
                    if (
                        modules.patch.patch_settings[pid].adaptive_cfg
                    ...
                    if freeu_enabled:
                        d.append(
                            (
                                "FreeU",
                                "freeu",
                                str((freeu_b1, freeu_b2, freeu_s1, freeu_s2)),
                            )
                        )
                    ...
                    d.append(
                        (
                            ...
                            "metadata_scheme",
                            (
                                metadata_scheme.value
                                if save_metadata_to_images
                                else save_metadata_to_images
                            ),
                        )
                    )
                    d.append(
                        ("Version", "version", "Fooocus v" + fooocus_version.version)
                    )
