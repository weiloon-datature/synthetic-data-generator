...
def download_models(
    ...
    if not args.always_download_new_model:
        if not os.path.exists(os.path.join(config.paths_checkpoints[0], default_model)):
            for alternative_model_name in previous_default_models:
                if os.path.exists(
                    os.path.join(config.paths_checkpoints[0], alternative_model_name)
                ):
                    print(
                        f"You do not have [{default_model}] but you have [{alternative_model_name}]."
                    )
                    print(
                        f"Fooocus will use [{alternative_model_name}] to avoid downloading new models, "
    ...
    for file_name, url in checkpoint_downloads.items():
        load_file_from_url(
            url=url, model_dir=config.paths_checkpoints[0], file_name=file_name
        )
    for file_name, url in embeddings_downloads.items():
        load_file_from_url(
            url=url, model_dir=config.path_embeddings, file_name=file_name
        )
    for file_name, url in lora_downloads.items():
        load_file_from_url(
            url=url, model_dir=config.paths_loras[0], file_name=file_name
        )
...
config.default_base_model_name, config.checkpoint_downloads = download_models(
    ...
    config.previous_default_models,
